    ...
    class LoginController extends Controller
    {
        ...
        public function index()
        {
            if (IS_POST) {
                ...
                $error = 100;
                if (empty($verify = trim(I('post.verify')))) {
                ...
                if (!checkEmail($email = trim(I('post.email')))) {
                    $this->ajaxReturn(array('error' => $error, 'message' => '邮箱格式不正确'));
                }
                if (empty($password = trim(I('post.password')))) {
                    $this->ajaxReturn(array('error' => $error, 'message' => '密码不能为空'));
                }

                if (empty($userInfoDetail = $this->model->userInfo($email))) {
